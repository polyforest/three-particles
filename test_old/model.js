import * as model from '../src/ParticleEffectModel.ts'
...
describe('model', () => {
    ...
    describe('randomFromRange-linear', () => {
        const r =
            /** @type {ParticleEffectModel.ts.RangeModel} */ model.createRange(
                23,
                43,
            )
        it('Should return min when rng() provides 0.0', () => {
            assert.strictEqual(
                model.valueFromRange(r, () => 0),
                23,
            )
        })
        it('Should return max when rng() provides 1.0', () => {
            assert.strictEqual(
                model.valueFromRange(r, () => 1),
                43,
            )
        })
        it('Should return mid when rng() provides 0.5', () => {
            assert.strictEqual(
                model.valueFromRange(r, () => 0.5),
                33,
            )
        })
    })
    ...
    describe('randomFromRange-pow2In', () => {
        const r = model.createRange(23, 63, 'pow2In')
        it('Should return min when rng() provides 0.0', () => {
            assert.strictEqual(
                model.valueFromRange(r, () => 0),
                23,
