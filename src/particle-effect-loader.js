...
import {parseParticlesMaterialParams, ParticlesMaterial, particlesMaterialParamDefaults} from './particles-material.js';
...
export class ParticleEffectLoader extends Loader {
   ...
   *
   * @param {Partial<import('./model').ParticleEffectVo>} json A parsed JSON object.
    ...
    if (json.emitters !== undefined) {
      for (const emitterJson of json.emitters) {
        if (emitterJson.material) {
          const type = emitterJson.material.type;
          if (type === undefined || type === 'ParticlesMaterial') {
            const materialParams = parseParticlesMaterialParams(emitterJson.material, this.textures);
            emitterJson.material =
              new ParticlesMaterial(defaults(materialParams, particlesMaterialParamDefaults));
          } else {
            emitterJson.material = this._materialLoader.parse(emitterJson.material);
          }
        }
